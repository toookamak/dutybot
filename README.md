...
| 配置 | 必要性 | 未配置时的行为 |
| --- | --- | --- |
| 看守名单（`watch.json`） | 可选，允许空名单 | 设备状态、CPU 前五、读写前五、重启本机、僵尸清理、SSH 通知、CPU/磁盘/网络/根分区告警仍可用。服务菜单为空；无服务崩机、假活、意外重启、孤儿 worker 对象。Web 日志仅可查询 `dutybot` 自身。随后可通过 Telegram 或 Web 添加服务。 |
| 单条记录的 `probe`（`host:port`） | 可选 | 该服务不做假活检测；状态、日志、重启、崩机与恢复通知仍可用。 |
...
应用代码已纳入本仓库。在仓库根目录以 root 执行 `sudo ./install.sh` 即可安装。
